...
void writeLoop(std::vector<leafinfo*> LV){
	...
	outputFile <<"	TFile *f = TFile::Open(\"MassConstraint2C_calibrated.root\");"<<std::endl;
	outputFile <<" 	TFile *f2 = new TFile(\"test2C_cal.root\",\"RECREATE\");"<<std::endl;
	outputFile <<"	f->GetObject(\"tree\",tree);"<<std::endl;
...
void generateTreeClass(){
	...
	TFile *f = TFile::Open("MassConstraint2C_calibrated.root");
